...
clip2 <- as(extent(window["xmin"], window["xmax"],
                   window["ymin"], window["ymax"]), "SpatialPolygons")         # Create cropping window for land
...
overlay <- generate_polygon_overlay(domain, linecolor = "black", extent = attr(base3, "extent"),
                                    linewidth = 20, palette = "yellow", width = (nrow(mat)*1), height = (ncol(mat)*2.3))

overlay_l <- generate_line_overlay(readRDS("./cache/coast.rds"), attr(base3, "extent"), heightmap = base3, 
                                   color = "black", linewidth = 15, width = (nrow(mat)*1), height = (ncol(mat)*2.3))

mat %>% 
  sphere_shade(texture = create_texture("#ffffff", "#ffffff", "#ffffff",        # Create a pale colour scale 
                                        "#ffffff", "#ffffff")) %>%  
#  height_shade(texture = col) %>%                                              # Colour by height
  add_overlay(overlay_l) %>% 
  add_overlay(overlay) %>% 
#  plot_map()  
plot_3d(heightmap = mat, zscale = 10, 
        solidcolor = "#ffffff", solidlinecolor = "#ffffff",
        background = "#ffffff", theta = 0, phi = 89, asp = 2.3, zoom = 0.85,
        windowsize = 1000)

tictoc::tic()
render_highquality(lightaltitude = 55, lightdirection = 45, sample_method = "sobol",
