...
domain <- rasterFromXYZ(data[["North Sea"]][,c(2,3,5)], digits = 3) %>% 
  ...
  left_join(labels) %>% 
  transmute(Class = case_when(Type == "Offshore Fine" ~ "D1",
...
ggplot() +
  geom_sf(data = line) +
  geom_sf(data = domain, fill = NA, colour = "red")
...
domain %>% 
  ...
  saveRDS("./cache/North Sea Habitats.rds")
...
as.data.frame(rock) %>% 
  ...
  saveRDS("./cache/North Sea Rock.rds")

rock_p <- ggplot() +
  geom_sf(data = land, fill = "black", colour = "black", size = 0.25) +
  geom_sf(data = domain, fill = "white", colour = "white", size = 0.3) +
...
hab_p <- ggplot() +
  geom_sf(data = land, fill = "white", size = 0.25) +
  geom_sf(data = domain, aes(fill = Class), size = 0.01, colour = "white") +
  geom_sf(data = land, fill = "white", colour ="#081a24", size = 0.05) +
  geom_sf(data = line, colour = "#B01313") +
  scale_fill_manual(values = G_Y2) +
  coord_sf(ylim = c(window["ymin"], window["ymax"]), xlim = c(window["xmin"], window["xmax"]), expand = F) +
  theme_minimal() +
  theme(text = element_text(family = "Avenir", size = 9, colour = "white"),
        axis.text = element_text(colour = "white"),
        panel.border = element_rect(colour = "black", fill = NA, size = 1),
        legend.position = c(0.82, 0.075),
        legend.key.size = unit(0.3, "cm")) +
  guides(fill = guide_legend(ncol = 2, title.hjust = 0.5, title.position = "left",
...
rock_p <- ggplot() +
  ...
  geom_sf(data = domain, fill = "white", colour = "white", size = 0.3) +
  geom_stars(data = rock) +
  geom_sf(data = domain, fill = NA, colour = "#284481", size = 0.01) +
  geom_sf(data = land, fill = "white", colour ="#081a24", size = 0.05) +
  geom_sf(data = line, colour = "#B01313") +
  scale_fill_gradient(low = "white", high = "#8c17ae", na.value = NA, labels = scales::percent) +
  coord_sf(ylim = c(window["ymin"], window["ymax"]), xlim = c(window["xmin"], window["xmax"]), expand = F) +
...
hab_p + rock_p & theme_minimal() & theme(text = element_text(family = "Avenir", size = 9, colour = "white"),
      axis.text = element_text(colour = "white"),
      panel.border = element_rect(colour = "black", fill = NA, size = 1),
      legend.position = c(0.82, 0.075),
      legend.key.size = unit(0.3, "cm"))
