...
hab_p <- ggplot() +
  ...
  labs(fill = "Habitats",
#       subtitle = "The West of Scotland Model Domain",
       x = NULL, y = NULL) +
  annotate("text", label = "200 m", x = window["xmin"]+ 6 , y = window["ymax"]- 2, 
...
domain %>% 
  ...
  saveRDS("./cache/W Scotland Habitats.rds")
...
as.data.frame(rock) %>% 
  ...
  saveRDS("./cache/W Scotland Rock.rds")
